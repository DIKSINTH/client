...
export default function ViewIosDevelopment() {
  const [data, setData] = useState(null);

  useEffect(() => {
    fetch("http://localhost:5000/api/iosdevelopment")
      .then((res) => res.json())
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
          <div>
            ...
            <p className="text-slate-500 text-sm font-medium">
              Manage Apple ecosystem application service content and display
              assets.
            </p>
          </div>
                ...
                {data ? (
                  <tr className="group hover:bg-blue-50/30 transition-all duration-300">
                    ...
                    <td className="px-8 py-10">
                      <div className="relative w-48 h-28 overflow-hidden rounded-2xl border-4 border-white shadow-md group-hover:shadow-blue-200/50 transition-all">
                        {data.Image ? (
                          <img
                    ...
                    <td className="px-8 py-10">
                      <div className="max-w-md">
                        <h2 className="text-xl font-extrabold text-slate-900 group-hover:text-blue-600 transition-colors mb-2 italic">
                          {data.Title}
                        </h2>
                        ...
                        <div className="flex gap-2 mt-4">
                          <span className="px-2 py-1 bg-slate-100 text-[10px] font-bold text-slate-500 rounded-lg uppercase tracking-tight flex items-center gap-1">
                            <FiInfo size={12} /> Premium iOS Service
                          </span>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-8 py-10 text-right">
                      <Link
                        to="/dashboard/view-iosdevelopment/edit"
                        className="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-xl text-sm font-bold hover:bg-blue-600 transition-all shadow-lg active:scale-95 group/btn"
                      >
                        <FiEdit3 size={16} />
